...
export async function POST(request: Request) {
  try {
    ...
    if (!packageId) return NextResponse.json({ success: false, error: 'packageId requerido' }, { status: 400 });
    ...
    if (!pkg) return NextResponse.json({ success: false, error: 'Paquete no encontrado' }, { status: 404 });
