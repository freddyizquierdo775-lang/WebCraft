...
export async function POST(request: Request) {
  try {
    ...
    if (!projectId || !elementHtml || !prompt) {
      return NextResponse.json({ success: false, error: 'Faltan projectId, elementHtml o prompt' }, { status: 400 });
    }
    ...
    const { data: project } = await supabase.from('user_projects').select('id,owner_id,html_content').eq('id', projectId).eq('owner_id', user.id).single();
    ...
    const openRouterRes = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'Eres un editor HTML experto. Recibes un fragmento HTML y una instrucción. Devuelve SOLO el HTML modificado (sin explicaciones, sin markdown, sin bloques de código). Mantén los mismos atributos y estructura base. Solo modifica lo solicitado.' },
    ...
    const modifiedHtml = (aiData.choices?.[0]?.message?.content || '').replace(/```html\n?|```\n?/g, '').trim();

    if (!modifiedHtml || modifiedHtml.length < 5) {
      return NextResponse.json({ success: false, error: 'La IA devolvió una respuesta vacía' }, { status: 502 });
    }
    ...
    await supabase.rpc('deduct_credits', {
      p_user_id: user.id, p_amount: 1, p_type: 'granular_edit',
      p_reference: { project_id: projectId, prompt },
      p_idempotency_key: `granular_${projectId}_${Date.now()}`,
    });

    await supabase.from('generation_logs').insert({
      project_id: projectId, user_id: user.id, prompt,
      model_used: 'google/gemini-flash-1.5',
      tokens_in: aiData.usage?.prompt_tokens || 0,
      tokens_out: aiData.usage?.completion_tokens || 0,
      credits_cost: 1, duration_ms: duration, success: true,
    });

    const { data: updatedProfile } = await supabase.from('profiles').select('credits_balance').eq('id', user.id).single();

    return NextResponse.json({
      success: true,
      data: { modifiedHtml, fullHtml, credits_used: 1, credits_remaining: updatedProfile?.credits_balance || 0 },
    });
  } catch (err) {
