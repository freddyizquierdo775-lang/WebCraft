    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="w-full max-w-sm rounded-2xl border bg-card p-6 shadow-2xl">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Tu sitio <strong>{projectName}</strong> estará disponible en internet con un subdominio
          gratuito de WebCraft.
        </p>
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <Button size="sm" onClick={onConfirm} disabled={loading}>
            {loading ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : null}Publicar ahora
          </Button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="w-full max-w-sm rounded-2xl border bg-card p-6 shadow-2xl">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Conecta tu propio dominio a <strong>{projectName}</strong>.
        </p>
        <div className="mt-4 space-y-3">
          ...
          />
          <textarea
            placeholder="Descripción meta (160 caracteres)"
            rows={2}
            className="w-full rounded-lg border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary"
          />
        </div>
...
export default function DashboardPage() {
  ...
  const handlePublish = useCallback(async () => {
    ...
    await supabase
      ...
      .update({
        status: 'published',
        published_url: `https://${publishTarget.name.toLowerCase().replace(/\s+/g, '-')}.webcraft.me`,
      })
  ...
  const handleDuplicate = useCallback(
    async (id: string) => {
      ...
      await supabase.from('user_projects').insert({
        ...
        briefing_data: original.briefing_data,
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (project.status === 'draft' || project.status === 'ready')
                          setPublishTarget({ id: project.id, name: project.name });
                      }}
