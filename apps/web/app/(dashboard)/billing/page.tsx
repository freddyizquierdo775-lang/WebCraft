...
export default function BillingPage() {
  ...
  useEffect(() => {
    ...
    supabase.from('credit_packages').select('*').eq('is_active', true).order('credits').then(({ data }) => {
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Card>
          ...
          <CardContent>
            ...
            <Progress value={Math.min((credits / monthlyCredits) * 100, 100)} className="mb-2 h-2" />
            <p className="text-xs text-muted-foreground">Plan {plan} · {monthlyCredits} créditos/mes</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {packages.map((pkg) => (
              ...
              >
                <span>{pkg.name} — ${(pkg.price_cents / 100).toFixed(0)} MXN</span>
                {checkingOut === pkg.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Zap className="h-4 w-4" />}
              </Button>
            ))}
          </CardContent>
        </Card>
            ...
            return (
              <Card key={key} className={isCurrent ? 'border-primary' : ''}>
                <CardHeader>
                  ...
                  <CardDescription>{p.price_monthly_mxn === 0 ? 'Gratis' : `$${p.price_monthly_mxn} MXN/mes`}</CardDescription>
                </CardHeader>
                <CardContent>
                  <ul className="mb-4 space-y-2 text-sm">
                    {p.features.map((f) => <li key={f} className="flex items-center gap-2"><Check className="h-3.5 w-3.5 text-primary" />{f}</li>)}
                  </ul>
                  <Button variant={isCurrent ? 'outline' : 'default'} size="sm" className="w-full" disabled={isCurrent} asChild={!isCurrent}>
